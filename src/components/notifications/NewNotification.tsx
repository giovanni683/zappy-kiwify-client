...
function renderMessagePreview(text: string) {
  ...
  const parts = text.split(regex);
    return parts.map((part, i) => {
      ...
      return <span key={i} style={{ color: '#000' }}>{part}</span>; // texto normal preto
    });
}
...
export function NewNotification({ onBack }: NewNotificationProps) {
  // Estado para ativar/desativar notificação
  const [isActive, setIsActive] = useState(false);
  // Custom dropdown state for Evento
  ...
  // Custom dropdown state for Conexão
  ...
  // Custom dropdown state for Setor de atendimento
  ...
  return (
      ...
      >
        ...
        <div
          className="flex flex-col items-start gap-4 rounded-2xl shadow-none p-0 min-h-[600px]"
          style={{
            marginTop: 70,
            width: 388,
            padding: 0,
        ...
        >
          ...
          {/* Card de informação igual EditNotification */}
